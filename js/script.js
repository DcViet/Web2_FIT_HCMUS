...
const poems = [
     {
        title: 'Xây dựng Website theo yêu cầu',
        poem: "link",
        poet: 'Web2',
...
function twShare(url, title, winWidth, winHeight) {
    const winTop = 300;
    const winLeft = 300;
    window.open(`../index.php?text=${title}`, /* 'sharer' */'_blank', `top=${winTop},left=${winLeft},toolbar=0,status=0,width=${winWidth},height=${winHeight}`);
}

pen_id = $('._pen_id').text();

$('body').on('click', '.tw', () => {
    twShare(`#`, `Check out this animated version of "${poems[index].title}" by ${poems[index].poet} on /?slide=${index} %23poetryWithCSS`, 520, 350);
    return false;
});
